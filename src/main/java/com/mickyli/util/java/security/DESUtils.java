 ...
 *   <B>说       明</B>:DES工具类。
 *   
 *   <p>该类提供基于DES算法的加解密功能(对称加密)。
 *   <p>该类基于JDK6实现，DES算法支持如下:
 *   <li>密钥长度:默认56。
...
public class DESUtils {
	 ...
	 * 
	 * @param data 待加密的数据。
	 * @param key 字节数组形式的密钥(数组长度必须不能小于8位)。
	 * @param transformation 转换名称，例如 DES/CBC/PKCS5Padding。
	...
	/**
	 * 对数据进行加密处理。
	 * 使用默认的转换名称:DES/CBC/PKCS5Padding。
	 * 
	 * @param data 待加密的数据。
	 * @param key 字节数组形式的密钥(数组长度必须不能小于8位)。
	 * @return
	 ...
	 * @return
	 *      密钥对象。
	 * @throws IllegalArgumentException 如果给定的密钥内容少于8个字节。
	 */
	private static Key toKey(byte[] key){
		try {
			Assert.notNull(key, "字节形式的密钥不能为null!");
			Assert.isTrue(key.length >= 8, "字节形式密钥的数组长度不能小于8位!");
			DESKeySpec desKeySpec = new DESKeySpec(key);
			return EncryptionBase.toKey(desKeySpec, ALGORITHM);
